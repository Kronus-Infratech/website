...
function SearchParamSync({
    ...
    useEffect(() => {
        ...
        setSearch(qLocation && qLocation !== "All Locations" ? qLocation : "");
        setSelectedType(TYPES.includes(qType as (typeof TYPES)[number]) ? qType : "All");
        setSortBy(
            qBudget.includes("Low") || qBudget.includes("Under")
                ? "price-asc"
                : qBudget.includes("High") || qBudget.includes("Above")
                  ? "price-desc"
                  : "newest"
        );
        setShowFilters(Boolean(qType || qLocation || qBudget));
    }, [searchParams, setSearch, setSelectedType, setSortBy, setShowFilters]);
...
function ProjectsGrid({ projects }: { projects: Property[] }) {
    ...
    const filtered = useMemo(() => {
        ...
        return result;
    }, [search, selectedType, selectedStatus, sortBy, projects]);
